...
function readConvo(filename) {
  ...
  return new Promise(function(readConvoResolve, readConvoReject) {
    readFile(convofilename).then((content) => {
      var fileLines = content.toString().split(EOL);

      let convoList = fileLines.reduce(function(a, l, i) {
        if (l.startsWith('=')) {
          a.push(i);
        }
        ...
        if (idx + 1 < arr.length){
          let convLines = fileLines.slice(el, arr[idx + 1] + 1);
          return convLines;
        } else {
          return [];
        }
      ...
      conversationList.splice(-1, 1);
      readConvoResolve(conversationList);
