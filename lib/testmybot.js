    ...
    testcaseCb(testcaseFile, function(testcaseDone) {
      ...
      convo.readConvo(testcaseFile).then(
        (testcasesinfile) => {
          ...
          async.eachSeries(testcasesinfile, (testcase, convoDone) => {

            //log.info('running test ' + count);
            count = count + 1;


          async.eachSeries(testcase.conversation,
            (convomsg, convomsgDone) => {
              //log.info('CONVERSATION = ' + util.inspect(testcase.conversation));
              //log.info('convomsg = ' + util.inspect(convomsg));
            ...
            (err) => {
              if (err) {
                log.info(testcase.name + ':: Test failed: ' + err);
                failCb(err);
              }
              convoDone(err);
